...
/* Return -1 (non-zero) on error, 0 on success. */
LOCAL(int)
	 ...
	 */
{
	...
	for (argn = 1; argn < argc; argn++) {
		...
		if (keymatch(arg, "arithmetic", 1)) {
			...
			if (select_transform(JXFORM_DROP))
				return -1;
#else
			if (select_transform(JXFORM_NONE))    /* force an error */
				return -1;
#endif
			...
			if (keymatch(argv[argn], "horizontal", 1)) {
				if (select_transform(JXFORM_FLIP_H))
					return -1;
			}
			else if (keymatch(argv[argn], "vertical", 1)) {
				if (select_transform(JXFORM_FLIP_V))
					return -1;
			}
			else {
				return usage();
			...
			if (keymatch(argv[argn], "90", 2)) {
				if (select_transform(JXFORM_ROT_90))
					return -1;
			}
			else if (keymatch(argv[argn], "180", 3)) {
				if (select_transform(JXFORM_ROT_180))
					return -1;
			}
			else if (keymatch(argv[argn], "270", 3)) {
				if (select_transform(JXFORM_ROT_270))
					return -1;
			}
			else {
				return usage();
			...
			if (select_transform(JXFORM_TRANSPOSE))
				return -1;

		}
		else if (keymatch(arg, "transverse", 6)) {
			/* Transverse transpose (across UR-to-LL axis). */
			if (select_transform(JXFORM_TRANSVERSE))
				return -1;
			...
			if (select_transform(JXFORM_WIPE))
				return -1;
#else
			if (select_transform(JXFORM_NONE))	/* force an error */
				return -1;
...
#define main(cnt, arr)      jpegturbo_jpegtran_main(cnt, arr)
#endif
